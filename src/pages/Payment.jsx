...
export default function Payment() {
  ...
  const dadosTela = location.state || {
    arenaId: 1, horarioId: 2, data: new Date().toISOString().split("T")[0],
    nomeArena: "Arena Society", horario: "18:00 - 1h", valor: 120,
  };
  ...
  const valorBase = Number(dadosTela.valor) || 120;
  ...
  const handleConfirmar = async () => {
    // 1. Cria a reserva
    const resultCriar = await dispatch(criarReservaThunk({
      arenaId:        dadosTela.arenaId,
      horarioId:      dadosTela.horarioId,
      data:           dadosTela.data,
      formaPagamento: FORMA_PAGAMENTO_MAP[formaPagamento],
      cupom:          cupomAplicado || null,
    }));

    if (criarReservaThunk.rejected.match(resultCriar)) return;

    const reservaId = resultCriar.payload.id;

    // 2. Confirma o pagamento
    const resultConfirmar = await dispatch(confirmarPagamentoThunk(reservaId));

    if (confirmarPagamentoThunk.fulfilled.match(resultConfirmar)) {
      setSnackbar(true);
  ...
  return (
    <Box sx={{ minHeight: "100vh", backgroundColor: "#f5f7f9", pb: 6 }}>
      {/* Header */}
      <Box sx={{ background: "linear-gradient(160deg, #166534 0%, #16a34a 60%, #22c55e 100%)", px: { xs: 2, sm: 3 }, pt: 3, pb: 5, position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between", color: "#fff" }}>
        <Button onClick={() => navigate(-1)} sx={{ minWidth: 0, color: "#fff", p: 0.5, "&:hover": { backgroundColor: "rgba(255,255,255,0.15)" } }}>
          <ArrowBackIosNewIcon sx={{ fontSize: 18 }} />
        </Button>
        <Typography sx={{ fontWeight: 700, fontSize: { xs: "1.1rem", sm: "1.25rem" } }}>Pagamento</Typography>
        <Box sx={{ width: 32 }} />
      ...
      <Container maxWidth="sm" sx={{ px: { xs: 2, sm: 3 }, pt: 2 }}>
          ...
          <CardContent sx={{ p: "16px !important" }}>
            <Box sx={{ display: "flex", gap: 1.5 }}>
              <Box component="img"
                ...
                alt={dadosTela.nomeArena}
                sx={{ width: 80, height: 68, objectFit: "cover", borderRadius: "10px", flexShrink: 0 }} />
              <Box sx={{ flex: 1 }}>
                <Typography sx={{ fontWeight: 700, fontSize: "0.95rem", color: "#111827" }}>{dadosTela.nomeArena}</Typography>
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mt: 0.3 }}>
                  <StarIcon sx={{ fontSize: 13, color: "#f59e0b" }} />
                  <Typography sx={{ fontSize: "0.78rem", color: "#6b7280" }}>4.7 · 120 avaliações</Typography>
                </Box>
                <Typography sx={{ fontSize: "0.78rem", color: "#6b7280", mt: 0.2 }}>🕒 {dadosTela.horario}</Typography>
              </Box>
        ...
        {/* Forma de pagamento */}
        <Typography sx={{ fontWeight: 700, color: "#111827", fontSize: "0.95rem", mb: 1.5 }}>Forma de Pagamento</Typography>
          ...
          {[
            ...
            <Card key={op.value} sx={paymentCardSx(op.value)}>
              <CardContent sx={{ py: "12px !important", px: 1.5 }}>
                <FormControlLabel value={op.value}
                  ...
                  label={
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1.2 }}>
                      {op.icon}
                      <Box>
                        <Typography sx={{ fontWeight: 700, color: "#111827", fontSize: "0.9rem" }}>{op.label}</Typography>
                        {op.sub && <Typography sx={{ fontSize: "0.75rem", color: "#6b7280" }}>{op.sub}</Typography>}
                      </Box>
                    </Box>
                  } />
              </CardContent>
        ...
        <Box sx={{ display: "flex", gap: 1, mb: 0.5 }}>
          <TextField placeholder="Digite seu cupom" fullWidth value={cupom}
          ...
          <Button variant="contained" onClick={handleAplicarCupom} disabled={!cupom.trim()}
            sx={{ borderRadius: "12px", minWidth: 100, height: 46, textTransform: "none", fontWeight: 700, flexShrink: 0, backgroundColor: "#16a34a", "&:hover": { backgroundColor: "#15803d" }, "&.Mui-disabled": { backgroundColor: "#d1d5db" } }}>
            Aplicar
          </Button>
        </Box>
        ...
        {cupomErro && (
          <Box sx={{ display: "flex", alignItems: "center", gap: 0.75, mb: 1.5 }}>
            ...
            <Typography sx={{ fontSize: "0.8rem", color: "#ef4444", fontWeight: 600 }}>Cupom inválido ou expirado.</Typography>
          </Box>
        )}
        ...
        <Button fullWidth variant="contained" onClick={handleConfirmar} disabled={loading}
          sx={{ height: 50, borderRadius: "14px", backgroundColor: "#16a34a", textTransform: "none", fontWeight: 700, fontSize: "1rem", boxShadow: "0 6px 20px rgba(22,163,74,0.30)", "&:hover": { backgroundColor: "#15803d" } }}>
          {loading ? <CircularProgress size={22} sx={{ color: "#fff" }} /> : "Confirmar pagamento"}
        </Button>
