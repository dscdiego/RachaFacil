...
export default function Register() {
  ...
  useEffect(() => {
    if (isLoggedIn) navigate("/buscar", { replace: true });
  }, [isLoggedIn, navigate]);
  ...
  const hasRealErrors = Boolean(
    ...
    validatePhone(form.telefone) || validateStrongPassword(form.senha)
  );
  ...
  const handleRegister = () => {
    ...
    dispatch(cadastrarUsuarioThunk({
      nome: form.nome.trim(),
  ...
  return (
    <Box sx={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <Box sx={{ background: "linear-gradient(160deg, #166534 0%, #16a34a 60%, #22c55e 100%)", px: 2.5, pt: 3, pb: 5, position: "relative", display: "flex", alignItems: "center", gap: 1.5 }}>
        <IconButton onClick={() => navigate("/")} sx={{ color: "#fff", p: 0.5 }}>
          <ArrowBackIosNewIcon sx={{ fontSize: 18 }} />
        </IconButton>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <SportsSoccerIcon sx={{ fontSize: 22, color: "#fff" }} />
          <Typography sx={{ fontWeight: 800, fontSize: "1.15rem", color: "#fff" }}>Criar conta</Typography>
        </Box>
      ...
      <Box sx={{ backgroundColor: "#f5f7f9", flex: 1, px: { xs: 2.5, sm: 4 }, pt: 2.5, pb: 4, maxWidth: 440, width: "100%", mx: "auto" }}>
