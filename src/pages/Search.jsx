...
import {
  Box, Typography, TextField, InputAdornment, Card, CardContent, Chip
} from "@mui/material";
...
const arenasMock = [
  {
    ...
    preco: 70,
    imagem: "https://images.unsplash.com/photo-1599058917212-d750089bc07e?auto=format&fit=crop&w=900&q=80",
  },
  {
    id: 2,
    nome: "Arena Fut7",
    ...
    preco: 100,
    imagem: "https://images.unsplash.com/photo-1508098682722-e99c643e7485?auto=format&fit=crop&w=900&q=80",
  },
  {
    id: 3,
  ...
  const arenasFiltradas = useMemo(() => {
    ...
    return arenasMock.filter((arena) => {
      const bateBusca =
        ...
        arena.esportes.some((e) => e.toLowerCase().includes(texto));
      const bateCategoria =
        categoriaAtiva === "Todos" || arena.esportes.includes(categoriaAtiva);
  ...
  return (
    <Box sx={{ backgroundColor: "#f5f7f9", minHeight: "100vh", pb: 10 }}>
      ...
      >
        ...
        <TextField
          fullWidth placeholder="Buscar arenas ou esportes"
          value={busca} onChange={(e) => setBusca(e.target.value)}
          sx={{
            "& .MuiOutlinedInput-root": {
              backgroundColor: "#fff", borderRadius: "30px",
              height: { xs: 50, sm: 54 }, px: 1,
          ...
          return (
            <Chip
              ...
              icon={React.cloneElement(cat.icon, {
                sx: { fontSize: "16px !important", color: ativa ? "#fff !important" : "#6b7280 !important" },
              })}
              label={cat.label}
              onClick={() => setCategoriaAtiva(cat.label)}
              sx={{
                backgroundColor: ativa ? "#16a34a" : "#fff",
                color: ativa ? "#fff" : "#374151",
                border: ativa ? "1px solid #16a34a" : "1px solid #e5e7eb",
                borderRadius: "14px",
                height: 38,
                fontWeight: 600,
                fontSize: "0.8rem",
                ...
                "& .MuiChip-icon": { ml: 1 },
                "&:hover": { backgroundColor: ativa ? "#15803d" : "#f9fafb" },
              }}
            />
          );
        })}
      ...
      <Box sx={{ px: { xs: 2, sm: 2 } }}>
        <Typography sx={{ fontWeight: 700, fontSize: "1rem", mb: 2, color: "#111827" }}>
          {arenasFiltradas.length > 0
            ? `${arenasFiltradas.length} arena${arenasFiltradas.length > 1 ? "s" : ""} próxima${arenasFiltradas.length > 1 ? "s" : ""}`
            : "Resultados da busca"}
        </Typography>
          ...
          arenasFiltradas.map((arena) => (
            ...
            >
              <Box
                ...
                sx={{ width: 110, minHeight: 110, objectFit: "cover", flexShrink: 0 }}
              />
